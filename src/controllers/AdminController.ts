  ...
  return experience.projects ? experience.projects.find(
    ...
    (experienceProject: string) => projects.find((p: ProjectData) => (experienceProject == p._id
      && p.members ? p.members.find((m: Member) => m.roles.includes('admin')
        && m.userId === userData._id) !== undefined : false)) !== undefined,
  ) !== undefined : false;
}

export async function ensureAdminOfProjectForExperience(
  request: Request,
  response: Response,
  next: NextFunction,
) {
  ...
  if (request.query.projectId) {
    ...
    return response.json(res.filter((snap) => {
      ...
      return snap.projects
       && snap.projects.find((p: any) => p.toString() === request.query.projectId) !== undefined;
    }));
  }
